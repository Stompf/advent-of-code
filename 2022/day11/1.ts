...
class Monkey {
    ...
    constructor(lines: string[]) {
        ...
        console.log(this.trueTest);
        console.log(this.falseTest);
        console.log("====================");
    }

    doRound(monkeys: Array<Monkey>) {
        while (this.items.length > 0) {
            this.inspections++;
            const item = this.items.shift()!;
            item.worryLevel = eval(this.operation.replace(/old/g, String(item.worryLevel)));
