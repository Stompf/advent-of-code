        ...
        for (const overlapID of current.overlapping) {
            ...
            for (const rotation of rotations) {
                ...
                if (uniqueDifferences.size === 1) {
                    const [dx, dy, dz] = [...uniqueDifferences][0].split(',').map(Number) as Point;
            ...
            if (selectedRotation === undefined) {
                console.log(`Failed to find rotation for ${overlapID} with respect to ${current}`);
                return 0;
            }
...
const solve = async () => {
    ...
    setConnectionsAndCount(scanners);
    const result = findLargestManhattanDistance(scanners);
